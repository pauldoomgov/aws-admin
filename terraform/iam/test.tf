...
resource "aws_iam_policy_attachment" "ReadOnlyAccess_test_attach" {
  count      = "${var.is_test}"
...
resource "aws_iam_group_policy_attachment" "tts_default_mfa_test_attach" {
  count      = "${var.is_test}"
  group      = "${aws_iam_group.default_group.name}"
  policy_arn = "${aws_iam_policy.tts_mfa.arn}"
}

resource "aws_iam_group_policy_attachment" "tts_default_remote_test_attach" {
  count      = "${var.is_test}"
  group      = "${aws_iam_group.default_group.name}"
...
resource "aws_iam_policy" "tts_test_management_assume_org_account_access_role" {
  count       = "${var.is_test}"
